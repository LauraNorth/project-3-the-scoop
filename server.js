...
//begining of code dealing with comments.
function getComments(url, request) {
  const response = {};

  response.status = 200;
  response.body = {
    articles: Object.keys(database.comments)
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.url &&
    ...
    database.users[comment.articleID].commentIds.push(comment.id);
...
function deleteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[id];
  const articleID = database.comments.articleID;
  const userID = database.comments.username;
  let array;
  let index;
  const response = {};
...
//this is meant to first find the index of this comments ID in the array
//database.articles.commentsIDs, and then splice it out.
    array = database.articles[articleID].commentsIDs;
    index = array.indexOf(savedComment);
    if(index > -1) {
      array.splice(index, 1);
    }
//this is meant to first find the index of this comments ID in the array
//database.users.commentsIDs, and then splice it out.
    array = database.user[username].commentsIDs;
    index = array.indexOf(savedComment);
    if(index > -1) {
      array.splice(index, 1);
    }

    const userCommentIds = database.users[savedComment.id].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    response.status = 204;
  } else {
    response.status = 400;
...
//end of code dealing with comments.
